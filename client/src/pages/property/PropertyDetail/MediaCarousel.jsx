...
export default function MediaCarousel(props) {
	...
	return (
		<>
			{imagePanel}
			<Tabs className='property-item__gallery' value={value} onChange={handleChange} variant='scrollable' scrollButtons allowScrollButtonsMobile aria-label='scrollable tabs' sx={{ m: 0 }}>
				<Tab
					className='property-item__media'
					sx={{ m: 0 }}
					aria-controls={`full-width-tabpanel-${0}`}
					id={`full-width-tab-${0}`}
					label={<img src={property?.details?.cover_image_url} alt='thumbnail-gallery' />}
				/>
				{imageTabs}
				<Button onClick={() => props.onOpen()} sx={{ width: '8rem', ml: 1, height: '5rem', alignSelf: 'center' }} variant='outlined'>
					<AddIcon />
				</Button>
			</Tabs>
		</>
